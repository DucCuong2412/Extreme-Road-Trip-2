...
public class Detonator : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            _fireball = (base.gameObject.AddComponent(typeof(DetonatorFireball)) as DetonatorFireball);
            _fireball.Reset();
        }
        if (!_smoke && autoCreateSmoke)
        {
            _smoke = (base.gameObject.AddComponent(typeof(DetonatorSmoke)) as DetonatorSmoke);
            _smoke.Reset();
        }
        if (!_sparks && autoCreateSparks)
        {
            _sparks = (base.gameObject.AddComponent(typeof(DetonatorSparks)) as DetonatorSparks);
            _sparks.Reset();
        }
        if (!_shockwave && autoCreateShockwave)
        {
            _shockwave = (base.gameObject.AddComponent(typeof(DetonatorShockwave)) as DetonatorShockwave);
            _shockwave.Reset();
        }
        if (!_glow && autoCreateGlow)
        {
            _glow = (base.gameObject.AddComponent(typeof(DetonatorGlow)) as DetonatorGlow);
            _glow.Reset();
        }
        if (!_light && autoCreateLight)
        {
            _light = (base.gameObject.AddComponent(typeof(DetonatorLight)) as DetonatorLight);
            _light.Reset();
        }
        if (!_force && autoCreateForce)
        {
            _force = (base.gameObject.AddComponent(typeof(DetonatorForce)) as DetonatorForce);
            _force.Reset();
        }
        if (!_heatwave && autoCreateHeatwave && SystemInfo.supportsImageEffects)
        {
            _heatwave = (base.gameObject.AddComponent(typeof(DetonatorHeatwave)) as DetonatorHeatwave);
            _heatwave.Reset();
        }
        components = GetComponents(typeof(DetonatorComponent));
    }
    ...
    private void UpdateComponents()
    {
        ...
        if (components == null || components.Length == 0)
        {
            ...
            if (components == null || components.Length == 0)
            {
                Debug.LogWarning("No DetonatorComponents found on this object.");
                return;
            }
        }
    ...
    public void Explode()
    {
        ...
        //if(components == null || components.Length == 0)
        //{
        //    Debug.Log("đã return nì null"); return;
        //}
