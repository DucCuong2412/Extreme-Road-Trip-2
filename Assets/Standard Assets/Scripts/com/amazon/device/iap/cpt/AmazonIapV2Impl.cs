	...
	public abstract class AmazonIapV2Impl : MonoBehaviour, IAmazonIapV2
	{
		private abstract class AmazonIapV2Base : AmazonIapV2Impl
		{
			...
			private string GetUserDataJson(string jsonMessage)
			{
				...
				stopwatch.Stop();
				logger.Debug($"Successfully called native code in {stopwatch.ElapsedMilliseconds} ms");
				return result;
			}
		...
		{
			public new static AmazonIapV2UnityAndroid Instance => AmazonIapV2UnityBase.getInstance<AmazonIapV2UnityAndroid>();
		...
		public static IAmazonIapV2 Instance => Builder.instance;
